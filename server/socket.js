 ...
 *  1. Activity Group Chat  ΓÇö room name: `activity_<activityId>`
 *     Members and the host can send, delete, and pin messages.
 *
 *  2. Direct Messages (DMs) ΓÇö room name: `dm_<userId1>_<userId2>` (sorted)
...
const initializeSocket = (server) => {
  const io = new Server(server, {
    ...
    console.log(`≡ƒöî Socket connected: ${socket.id} (user: ${socket.user.id})`);
    ...
    socket.join(`user_${socket.user.id}`);
      ...
      try {
        ...
        if (!activity) {
          console.warn(`Γ¥î join-activity-room: activity ${activityId} not found`);
          return;
        }

        const isHostOrMember = member || activity?.hostId === socket.user.id;
        if (!isHostOrMember) {
          console.warn(`Γ¥î Unauthorised room join by user ${socket.user.id} for activity ${activityId}`);
          return;
        }

        socket.join(`activity_${activityId}`);
        console.log(`≡ƒƒó User ${socket.user.id} joined activity_${activityId}`);
      } catch (err) {
    ...
    /** leave-activity-room ΓÇö called when the user navigates away */
    ...
    socket.on("send-activity-message", async ({ activityId, content }) => {
      console.log(`≡ƒÆ¼ Processing activity message from ${socket.user.id} to room activity_${activityId}`);
      try {
    ...
    /** delete-activity-message ΓÇö broadcast deletion event, client removes from UI */
    ...
    /** pin-activity-message ΓÇö broadcast pin toggle to the room */
